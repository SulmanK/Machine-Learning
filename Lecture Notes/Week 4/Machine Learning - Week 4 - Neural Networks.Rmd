---
title: 'Machine Learning: Week 4 - Neural Networks'
author: "Sulman Khan"
...
We label these intermediate or "hidden" layer nodes \(a^2_0 ... a_n^2\) and call them  "activation units."
...
$$z_k^{(2)}) = \Theta^{(1)}_{k,1} x_1 + ... + \Theta^{(1)}_{k,n}x_n$$

The vector representation of x and \(z^{j}\) is:

$$\begin{align*}x = \begin{bmatrix}x_0 \newline x_1 \newline\cdots \newline x_n\end{bmatrix} &z^{(j)} = \begin{bmatrix}z_1^{(j)} \newline z_2^{(j)} \newline\cdots \newline z_n^{(j)}\end{bmatrix}\end{align*}$$

Setting \(z = a^{(1)}\), we can rewrite the equation as:
...
$$ \Theta^{(1)} = \left [\matrix {-30 & 20 & 20} \right ] $$
...
$$ y^{(i)} = \left [ \matrix {1 \\ 0 \\ 0 \\ 0}  \right ], \left [ \matrix {0 \\ 1 \\ 0 \\ 0}  \right ], \left [ \matrix {0 \\ 0 \\ 1 \\ 0}  \right ], \left [ \matrix {0 \\ 0 \\ 0 \\ 1}  \right ] $$