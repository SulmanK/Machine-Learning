---
title: 'Machine Learning: Week 1 - Introduction'
author: "Sulman Khan"
...
Note that this is like the equation of a straight line. We give to \(h_\theta(x)\) values for \(\theta_0\) and \(\theta_1\) to get our estimated output \(\hat{y}\). In other words, we are trying to create a function called \(h\theta_0\) that is trying to map our input data (the x's) to our output data (the y's).
...
$$\theta_j := \theta_j - \alpha[ Slope\  of \ tangent \ aka \ derivative \ in \  j \ dimension ][Slope \ of \ tangent \ aka \ derivative \ in \ j \ dimension]$$
...
Note that we have separated out the two cases for \(\theta_j\) into separate equations for \(\theta_0\) and \(\theta_1\) and that for \(\theta_1\) we are multiplying \(x_1\) at the end due to the derivative.
...
$$  \left[\matrix{a & b & c \\ d & e & f \\ g & h & i \\ j & k & l}\right]$$
The above matrix has four rows and three columns, so it is a 4 x 3 matrix.

A vector is a matrix with one column and many rows:

$$ \left[\matrix{w \\ x \\ y \\ z}\right]$$
...
$$ \left[\matrix{a  & b \\ c & d }\right] + \left[\matrix{w  & x \\ y & z }\right] = \left[\matrix{a + w  & b + x \\ c + y & d + z l}\right]$$ 

To add or subtract two matrices, their dimensions must be the same.

In scalar multiplication, we simply multiply every element by the scalar value:

$$ \left[\matrix{a  & b \\ c & d }\right] * x = \left[\matrix{a*x  & b*x \\ c*x & d*x }\right]$$ 
...
$$ \left[\matrix{a  & b \\ c & d \\ e & f }\right] * \left[\matrix{x \\ y}\right] = \left[\matrix{a*x + b*y \\ c*x + d*y \\ e*x + f*y }\right]$$ 
...
$$ \left[\matrix{a  & b \\ c & d \\ e & f }\right] * \left[\matrix{w & x \\ y & z}\right] = \left[\matrix{a*w + b*y & a*x + b*z \\ c*w + d*y & c*x + d*z \\ e*w + f*y & e*x + f*z }\right]$$ 
...
$$ A =  \left[\matrix{a  & b  \\ c & d  \\ e & f }\right]$$

$$ A^T =  \left[\matrix{a  & c & e  \\ b & d  & f\\ }\right]$$
In other words:
$$ A_{ij} = A_{ji}^T$$