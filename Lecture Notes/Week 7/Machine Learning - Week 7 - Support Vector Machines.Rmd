---
title: 'Machine Learning: Week 7 - Support Vector Machines '
author: "Sulman Khan"
...
$$(\theta) =  \frac{1}{m} \sum_{i = 1}^{m} [y^{(i)} cost_1(\theta^T x^{(i)}) + (1 - y^{(i)}) cost_0 (\theta^T x ^{(i)})] + \frac{\lambda}{2m} \sum_{j = 1}^n \theta_j^2$$
...
$$(\theta) =  \sum_{i = 1}^{m} [y^{(i)} cost_1(\theta^T x^{(i)}) + (1 - y^{(i)}) cost_0 (\theta^T x ^{(i)})] + \frac{\lambda}{2} \sum_{j = 1}^n \theta_j^2$$

Furthermore, convention dictates that we regularize using a factor C, instead of \(\lambda\), like so:

$$(\theta) =  C \sum_{i = 1}^{m} [y^{(i)} cost_1(\theta^T x^{(i)}) + (1 - y^{(i)}) cost_0 (\theta^T x ^{(i)})] + \frac{1}{2} \sum_{j = 1}^n \theta_j^2$$
...
$$ h_\theta (x) = \cases{1 \ if \ \Theta^T x \geq 0 \\ 0 \ otherwise}$$
...
$$(\theta) =   \sum_{i = 1}^{m} [y^{(i)} cost_1(\Theta^T x) + (1 - y^{(i)}) cost_0 (\theta^T x )] = 0 $$
...
$$ u =  \left [ \matrix{u_1 \\ u_2}      \right ]    v =  \left [ \matrix{v_1 \\ v_2}      \right ]$$
...
Note that \(u^Tv = ||u|| \cdot ||v|| \cos \theta\)
where \(\theta\) is the angle between u and v. Also, \(p = ||v|| \cos \theta\). If you substitute p for \(||v|| \cos \theta \), you get \(u^Tv = p \cdot ||u||\).
...
$$\min_{\Theta} C \sum_{i=1}^m y^{(i)}\text{cost}_1(\Theta^Tf^{(i)}) + (1 - y^{(i)})\text{cost}_0(\theta^Tf^{(i)}) + \dfrac{1}{2}\sum_{j=1}^n \Theta^2_jmin$$
